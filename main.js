...
const casinoHandlers = {
  ...
  cashout(payload, source, origin) {
    ...
    if (!Number.isFinite(amount) || amount <= 0) {
      source.postMessage({ kind: CASINO_REPLY, version: CASINO_VERSION, type: 'cashout:err', message: 'Invalid amount' }, origin);
      return;
    }
    balance += amount;
    updateBalanceDisplay();
    ...
    source.postMessage({ kind: CASINO_REPLY, version: CASINO_VERSION, type: 'cashout:ok', balance }, origin);
  },

  // Game fragt Balance ab
  getBalance(payload, source, origin) {
    source.postMessage({ kind: CASINO_REPLY, version: CASINO_VERSION, type: 'balance', balance }, origin);
  },

  // optionale FX
  'fx:glow'(payload, source, origin) {
    runCashoutGlow(Number(payload?.durationMs) || 700);
    source.postMessage({ kind: CASINO_REPLY, version: CASINO_VERSION, type: 'fx:glow:ok' }, origin);
  },
